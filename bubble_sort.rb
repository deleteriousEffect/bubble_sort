...
def bubble_sort_by(array)
  ...
  loop do
    ...
    last_unsorted_index = array.length - (1 + number_of_passes)
    while last_unsorted_index > 0
      # If the result of the block is negative swap element positions.
      if (yield array[sorting_el], array[sorting_el + 1]) < 0
        array.insert(sorting_el + 1, array.delete_at(sorting_el))
        swapped = true
      end
      sorting_el += 1
...
print(bubble_sort_by(['hi', 'hello', 'hey', 'a', 'long sting']) do |left, right|
